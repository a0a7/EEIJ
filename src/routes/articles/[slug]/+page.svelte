...
<article class="mx-auto max-w-4xl space-y-10">
	<header class="space-y-4">
		...
		<h1 class="font-serif text-4xl leading-tight text-[var(--text-strong)] md:text-5xl">
			{data.article.title}
		</h1>
	...
	<section class="space-y-4">
		<h2 class="font-serif text-2xl text-[var(--text-strong)]">Abstract</h2>
	...
	<section class="grid gap-5 md:grid-cols-[1.4fr_1fr]">
		<div class="card p-5">
			<h2 class="font-serif text-2xl text-[var(--text-strong)]">Citation</h2>
			<p class="mt-3 text-sm leading-7 text-[var(--text-default)]">{data.article.citation}</p>
			<p class="mt-2 text-xs tracking-[0.16em] text-[var(--text-muted)] uppercase">
				DOI: {data.article.doi}
			</p>
		</div>
		<div class="card p-5">
			<h2 class="font-serif text-2xl text-[var(--text-strong)]">Download</h2>
			<p class="mt-3 text-sm leading-7 text-[var(--text-default)]">
				Access the article PDF formatted for citation and offline review.
			</p>
			<a
				href={`https://eeijournal.org/downloads/eeij-${data.article.slug}.pdf`}
				class="focus-ring mt-4 inline-flex rounded-full bg-[var(--accent)] px-5 py-2 text-sm text-[var(--accent-contrast)]"
				download
			>
				Download PDF
			</a>
